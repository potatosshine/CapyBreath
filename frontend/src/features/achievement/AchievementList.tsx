...
const AchievementList = () => {
  ...
  const handleCheckAchievements = async () => {
    ...
    try {
      ...
      await Promise.all([fetchCatalog({ category, rarity }), fetchMyAchievements()]);
    } finally {
      setChecking(false);
    }
  };
  ...
  return (
    <PageShell>
      <main className="max-w-6xl mx-auto p-6 md:p-10 space-y-6">
        ...
        <SectionCard>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <label className="block">
              ...
              <select
                value={category}
                onChange={event => setCategory(event.target.value as AchievementCategory | 'all')}
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-gray-700">Raridade</span>
              <select
                value={rarity}
                onChange={event => setRarity(event.target.value as AchievementRarity | 'all')}
                className="w-full rounded-xl border border-gray-300 px-3 py-2"
              >
        ...
        {loading ? (
          <SectionCard>Carregando conquistas...</SectionCard>
        ) : error ? (
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
            <div>
              ...
              {filteredUnlocked.length === 0 ? (
                <div className="mb-8 text-gray-600">Nenhuma conquista desbloqueada com os filtros atuais.</div>
              ) : (
                <ul className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredUnlocked.map(achievement => (
                    <li key={achievement.id}>
                      <AchievementItem achievement={achievement} unlocked onClick={() => void handleSelectAchievement(achievement)} />
                    </li>
                  ))}
                </ul>
              )}
              ...
              {filteredLocked.length === 0 ? (
                <div className="text-gray-600">Nenhuma conquista em progresso com os filtros atuais.</div>
              ) : (
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredLocked.map(achievement => (
                    <li key={achievement.id}>
                      <AchievementItem achievement={achievement} onClick={() => void handleSelectAchievement(achievement)} />
                    </li>
                  ))}
                </ul>
              )}
            ...
            <SectionCard className="h-fit lg:sticky lg:top-6">
              ...
              {detailLoading ? (
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-2xl font-bold">{selectedAchievement.icon} {selectedAchievement.name}</p>
                    <p className="text-sm text-gray-600 mt-1">{selectedAchievement.description}</p>
                  </div>
                  <div className="grid gap-2 text-sm">
                    <p><span className="font-semibold">Categoria:</span> {selectedAchievement.category}</p>
                    <p><span className="font-semibold">Raridade:</span> {selectedAchievement.rarity}</p>
                    <p><span className="font-semibold">Pontos:</span> {selectedAchievement.points}</p>
                    <p><span className="font-semibold">Status:</span> {selectedAchievement.unlocked ? 'Desbloqueada' : 'Em progresso'}</p>
                  </div>

                  {selectedAchievement.unlocked ? (
                    <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
                      Desbloqueada em {selectedAchievement.unlocked_at ? new Date(selectedAchievement.unlocked_at).toLocaleString('pt-BR') : 'data indisponível'}.
                    </div>
                  ) : selectedAchievement.progress ? (
                    <div className="rounded-lg bg-gray-50 border p-3 text-sm text-gray-700">
                      Progresso: {selectedAchievement.progress.current}/{selectedAchievement.progress.target} ({Math.round(selectedAchievement.progress.percentage)}%)
                    </div>
                  ) : null}
                  ...
                  <button
                    ...
                    className="rounded-lg border border-capy-secondary/40 px-4 py-2 text-sm font-medium hover:bg-capy-light/50"
                  >
                    Fechar detalhe
                  </button>
                </div>
              ) : (
                <p className="text-gray-600 text-sm">Clique em uma conquista para ver o detalhe completo.</p>
              )}
            </SectionCard>
          </div>
        )}
